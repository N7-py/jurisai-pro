...
(() => {
    ...
    async function queryHF(model, payload, retries = 2) {
        ...
        if (response.status === 401) {
            throw new Error('Invalid API token. Please check your Hugging Face API token in Settings.');
        }
        ...
        if (!response.ok) {
            ...
            throw new Error(err.error || `API error (${response.status}). Please try again.`);
        }
    ...
    function initDocumentReview() {
        ...
        btn.addEventListener('click', async () => {
            ...
            if (text.length < 50) {
                showError(result, 'Text is too short for meaningful summarization. Please provide more content (at least 50 characters).');
                return;
            }
